...
def load_image_gt(dataset, config, image_id, augment=False,
  ...
  #  mask = np.array([m for m in mask if set(np.unique(m).flatten()) != {0}])
    ...
    mini_shape = config.MINI_MASK_SHAPE
    toremovearray = []
    ...
    for i in range(mask.shape[-1]):
        ...
        m = m[y1:y2, x1:x2]
        if m.size == 0:
            toremovearray.append(i)

    for i in  toremovearray:  
...
def data_generator(dataset, config, shuffle=True, augment=True, random_rois=0,
    ...
    while True:
        try:
          ...
          #      print ('inputtt', (np.shape(np.array(inputs))))
            #    print ('outputssss', (np.shape(np.array(outputs))))
...
class MaskRCNN():
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers):
      ...
      #  print ('train_datasettrain_dataset:  ', train_dataset)
